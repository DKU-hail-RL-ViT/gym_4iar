...
class TreeNode(object):
    ...
    def update_recursive(self, leaf_value):
        ...
        flops = 0
        if self._parent:
            flops += self._parent.update_recursive(-leaf_value)
...
class MCTS(object):
    ...
    def _playout(self, env):
        ...
        if self.rl_model in "DQN":
            ...
            if len(available) > 0:
                ...
                idx_max = available[np.argmax(masked_leaf_value[available])]
                flops_ += len(masked_leaf_value[available])
                action_probs[idx_max] = 1

                # add epsilon to sensible moves and discount as much as it from the action_probs[idx_max]
                action_probs[available] += self.epsilon / len(available)
                action_probs[idx_max] -= self.epsilon
                flops_ += 2 * len(available)
        ...
        flops_ = node.update_recursive(-leaf_value)
        flops = flops + flops_

        return planning_depth, depth_times, width_times, flops
    ...
    def get_move_probs(self, env, game_iter, temp):  # state.shape = (5,9,4)
        ...
        for n in range(self._n_playout):  # for 400 times
            ...
            planning_depth, depth_fre, width_fre, flops = self._playout(env_copy)
            flops_ += flops * planning_depth

        # calc the move probabilities based on visit counts at the root node
        act_visits = [(act, node._n_visits)
                      for act, node in self._root._children.items()]
        flops_ += len(self._root._children)

        acts, visits = zip(*act_visits)
        visits_array = np.array(visits)
        flops_ += len(visits)

        log_visits = np.log(visits_array + 1e-10)
        scaled_visits = (1.0 / temp) * log_visits
        flops_ += len(visits) + 1  # N multiplications + 1 division

        # 6. Softmax calculation
        max_val = np.max(scaled_visits)
        flops_ += len(visits)  # Max operation iterates over N elements

        exp_values = np.exp(scaled_visits - max_val)
        flops_ += len(visits)  # One exp operation per element

        sum_exp = np.sum(exp_values)
        flops_ += len(visits)  # Sum operation over N elements
...
class MCTSPlayer(object):
    ...
    def get_action(self, env, game_iter=0, temp=1e-3, return_prob=0):  # env.state_.shape = (5,9,4)
        ...
        if len(sensible_moves) > 0:
            acts, probs, mcts_flops = self.mcts.get_move_probs(env, game_iter, temp)  # env.state_.shape = (5,9,4)
            flops += mcts_flops  # Add FLOPs from MCTS search
            flops += len(acts)
            ...
            if self._is_selfplay:
                ...
                flops += len(probs)
                # reset the root node
                flops_ = self.mcts.update_with_move(-1)
                flops += flops_
            if return_prob:
